...
void lora_init(void){
    ...
    // Configure LNA for better reception
    lora_write_reg(REG_LNA, 0x23); // Max LNA gain, boost on

    // Clear all IRQ flags
    lora_write_reg(REG_IRQ_FLAGS, 0xFF);

    // Map DIO0 => RxDone (for reception)
    lora_write_reg(REG_DIO_MAPPING1, MAP_DIO0_LORA_RXDONE);
...
int lora_receive_packet(uint8_t *buf, uint8_t max_len, int16_t *rssi, int8_t *snr) {
    // Set FIFO ptr to RX base
    lora_write_reg(REG_FIFO_ADDR_PTR, 0x00);
    ...
    lora_write_reg(REG_OPMODE, 0x85); // LoRa + RX Continuous
    ...
    while (1) {
        ...
        if (irq & IRQ_PAYLOAD_CRC_ERROR_MASK) {
            ...
            lora_write_reg(REG_OPMODE, 0x81); // Back to standby
            return -3; // CRC error
        }
        ...
        if (irq & IRQ_RX_DONE_MASK) {
            ...
            // Get RSSI and SNR
            if (rssi) {
                int16_t pkt_rssi = lora_read_reg(REG_PKT_RSSI_VALUE);
                *rssi = -157 + pkt_rssi; // For 915MHz band
            }
            if (snr) {
                int8_t pkt_snr = lora_read_reg(REG_PKT_SNR_VALUE);
                *snr = pkt_snr / 4; // SNR in dB
            }
            ...
            // Set FIFO pointer to received packet
            lora_write_reg(REG_FIFO_ADDR_PTR, rx_addr);
            
            // Read packet data
            for (uint8_t i = 0; i < rx_nb_bytes; i++) {
            ...
            return rx_nb_bytes; // Return number of bytes received
        }
        
        // Timeout check (30 seconds)
        if (to_ms_since_boot(get_absolute_time()) - t0 > 30000) {
            lora_write_reg(REG_OPMODE, 0x81); // Back to standby
            return -1; // Timeout
        }
...
int lora_packet_available(void) {
    uint8_t irq = lora_read_reg(REG_IRQ_FLAGS);
    return (irq & IRQ_RX_DONE_MASK) ? 1 : 0;
}

int lora_read_packet(uint8_t *buf, uint8_t max_len, int16_t *rssi, int8_t *snr) {
    uint8_t irq = lora_read_reg(REG_IRQ_FLAGS);
